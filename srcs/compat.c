...
/*   Updated: 2019/04/06 17:20:09 by lperron     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
int			binarize(t_map *map)
{
	...
	if (!(map->path_compat.matrixbin = malloc(sizeof(uint64_t *)
					* map->nb_path)))
		return (0);
	i = -1;
	while (++i < map->nb_path)
	{
		if (!(map->path_compat.matrixbin[i] = ft_memalloc(sizeof(uint64_t)
						* size)))
			return (0);
		j = -1;
		j = -1;
